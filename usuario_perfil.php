...
$id_usuario_visualizado = $_GET['id'];

// Se for o próprio perfil, redireciona para meu_perfil.php
if ($id_usuario_visualizado == $_SESSION['id']) {
...
// Buscar dados do usuário visualizado
$perfil = buscarUsuarioPorId($id_usuario_visualizado);
...
// Buscar postagens e comentários do usuário visualizado
$postagens = buscarPostagensUsuario($id_usuario_visualizado, $desde, $mais) ?? [];
$comentarios = buscarComentariosUsuario($id_usuario_visualizado) ?? [];
?>
<!DOCTYPE html>
<html lang="pt-BR">
...
<body>
          ...
          <div class="info">
            ...
            <?php else: ?>
              <p class="bio" style="color: #666;">
                Este usuário não adicionou uma descrição ainda...
              </p>
            <?php endif; ?>
          </div>
      ...
      <main class="hashtag-container">
        ...
        <aside class="sidebar-hashtag">
          ...
          <div class="sidebar-box filter-box">
            ...
            <form action="usuario_perfil.php" method="GET" id="filterForm">
              <input type="hidden" name="id" value="<?= $id_usuario_visualizado ?>">
          ...
          <div class="sidebar-box obras-box">
            ...
            <div class="mini-carousel">
              <?php
              // Buscar obras mais postadas pelo usuário visualizado
              ...
              $stmt_obras->bind_param("s", $id_usuario_visualizado);
              $stmt_obras->execute();
      ...
      tabs.forEach((tab) => {
        tab.addEventListener("click", () => {
          ...
          if (tabType === 'atividade') {
            ...
            console.log('Iniciando carregamento de favoritos do usuário...');
  ...
  `;
            ...
            // Buscar postagens favoritas via AJAX com ID do usuário visualizado
            const timestamp = new Date().getTime();
            fetch(`php/buscar_favoritos_usuario.php?user_id=<?= $id_usuario_visualizado ?>&t=${timestamp}`)
              .then(response => {
                if (!response.ok) {
              ...
              .then(favoritos => {
                if (!favoritos || favoritos.length === 0) {
                  panel.innerHTML = `
          <h2 style="grid-column: 1 / -1; color: var(--text); font-size: 20px; margin: 20px 0 10px;">Favoritos</h2>
          <p class="empty">Este usuário ainda não favoritou nenhuma postagem 👀</p>
        `;
                } else {
                  let favoritosHTML = `
          <div style="grid-column: 1 / -1;">
            <h2 style="color: var(--text); font-size: 20px; margin: 20px 0 10px;">Favoritos (${favoritos.length})</h2>
