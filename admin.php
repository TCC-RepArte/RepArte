...
<body>
    ...
    <main class="admin-container">
        ...
        <section class="admin-card">
            ...
            <p>Use esta ferramenta para excluir qualquer item pelo ID, mesmo sem denúncia.</p>
            <form action="php/admin_actions.php" method="POST" class="admin-form" onsubmit="return confirm('Tem certeza absoluta? Isso não pode ser desfeito.');">
                        ...
                        <tbody>
                            <?php while($row = $result->fetch_assoc()): ?>
                                <tr>
                                    ...
                                    <td><span class="badge badge-<?= $row['tipo_denuncia'] ?>"><?= ucfirst($row['tipo_denuncia']) ?></span></td>
                                    ...
                                    <td>
                                        ...
                                        <?php 
                                            ...
                                            $link = "#";
                                            if($row['tipo_denuncia'] == 'postagem') $link = "postagem.php?id=" . $row['id_item_denunciado'];
                                            if($row['tipo_denuncia'] == 'usuario') $link = "usuario_perfil.php?id=" . $row['id_item_denunciado'];
                                        ?>
                                        <?php if($link != "#"): ?>
                                            <a href="<?= $link ?>" target="_blank" class="btn-link"><i class="fas fa-external-link-alt"></i> Ver</a>
                                        <?php endif; ?>
                                    </td>
                                    <td class="actions-cell">
                                        <!-- Botão para excluir o ITEM denunciado (Aceitar denúncia) -->
                                        <form action="php/admin_actions.php" method="POST" style="display:inline;" onsubmit="return confirm('Isso vai apagar o ITEM denunciado permanentemente. Confirmar?');">
                                            <input type="hidden" name="action" value="delete_manual">
                                            <input type="hidden" name="id_item" value="<?= $row['id_item_denunciado'] ?>">
                                            <input type="hidden" name="tipo_item" value="<?= $row['tipo_denuncia'] ?>">
                                            <input type="hidden" name="id_denuncia" value="<?= $row['id'] ?>"> <!-- Para apagar a denúncia junto -->
                                            <button type="submit" class="btn-action btn-ban" title="Apagar Item e Denúncia"><i class="fas fa-check"></i> Aceitar (Apagar Item)</button>
                                        </form>
                                        ...
                                        <form action="php/admin_actions.php" method="POST" style="display:inline;">
                                            ...
                                            <button type="submit" class="btn-action btn-ignore" title="Ignorar Denúncia"><i class="fas fa-times"></i> Ignorar</button>
                                        </form>
                                    </td>
                                </tr>
                            <?php endwhile; ?>
